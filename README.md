...
###  Lua 脚本引擎
- 在 `Lugin/script/` 目录下放置 `.lua` 文件，通过 `/script run <文件名>` 执行
- 支持 Bukkit/Paper API 的全部访问权限
- 事件监听、消息广播、定时任务、Adventure API 等开箱即用

### 模块系统
- 在 `Lugin/module/` 目录下按模块组织，每个模块独立隔离
...
### 安装
1. 将 `Lugin.jar` 放入服务器的 `plugins/` 目录
2. 启动服务器，插件会在服务器根目录自动创建 `Lugin/` 文件夹
3. 目录结构：
   ```
   Lugin/
   ├── script/        # 独立 Lua 脚本
...
### 运行脚本
```lua
-- Lugin/script/hello.lua
broadcast("&aHello, Lugin!")
...
### 创建模块
`Lugin/module/example/manifest.yml`:
```yaml
name: "example"
version: "1.0"
main: "main.lua"
...
`Lugin/module/example/scripts/main.lua`:
```lua
function onEnable()
...
产物位于 `build/libs/Lugin-1.0-SNAPSHOT-all.jar`。
