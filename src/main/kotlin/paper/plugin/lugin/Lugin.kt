package paper.plugin.lugin

import org.bukkit.command.Command
import org.bukkit.command.CommandSender
import org.bukkit.plugin.java.JavaPlugin
import paper.plugin.lugin.module.ModuleLoader
import paper.plugin.lugin.module.ConfigManager
import paper.plugin.lugin.module.ModulePackager
import java.io.File

class Lugin : JavaPlugin() {
    ...
    override fun onEnable() {
        ...
        val luginDir = File(serverRoot, "Lugin")
        val subDirs = listOf("script", "module", "config", "packages")

        val existing = subDirs.map { File(luginDir, it) }.filter { it.exists() }
        val missing = subDirs.map { File(luginDir, it) }.filter { !it.exists() }
        ...
        when {
            ...
            existing.isEmpty() -> {
                ...
                logger.info("Lugin初始化完成")
            }
            else -> {
                missing.forEach { it.mkdirs() }
                logger.info("Lugin已修复受损文件夹")
            }
        }

        scriptDir = File(luginDir, "script")
        val scriptConfigManager = ScriptConfigManager(this)
        luaEngine = LuaEngine(this, scriptConfigManager)
        luaEngine.init()

        val moduleDir = File(luginDir, "module")
        val packagesDir = File(luginDir, "packages")
        ...
        when (args[0].lowercase()) {
            ...
            "list" -> {
                ...
                if (files.isNullOrEmpty()) {
                    ...
                    sender.sendMessage("§6=== Lugin 脚本列表 ===")
